...
/**********************************************************************
 * 函数名称： handle_pic
 * 功能描述： 完成图片识别工作，且串口发出打靶数据
 * 输入参数： PT_PixelDatas - 一个结构体,包涵读取摄像头的数据
			  PT_hand_pic - 一个结构体,包涵处理图片的主要参数
 * 输出参数：  
 * 返 回 值： 0 - 成功, 负数 - 失败
 ***********************************************************************/
...
{	
	...
	check_uart1(green_info);//检测串口数据

	if(10){
	//	printf("start\n");
		ret=get_info(ptPixelDatas,green_info); //获取图像信息
		if(ret)
			return -1;

	}else
		test(green_info);//仅供调试测试使用

 	//printf("point1\n");
	ret = find_laser(green_info);//找到激光点
	if(ret){
...
//	printf("point2\n");
	ret=otsu_method(green_info);//二值化数据
	if(ret){
...
//	 printf("point4\n");
	...
	//printf("point3\n");
	check_laser(green_info,bound_x1, bound_x2, bound_y1, bound_y2);//检测激光的落地点
    //printf("laser cope %d\n",green_info->laser_scope);
	
	find_coordinate(green_info,bound_x1, bound_x2,bound_y1,bound_y2);//识别坐标
//	printf("point4\n");
	calculate_grade(green_info,bound_x1, bound_x2,bound_y1,bound_y2);//计算成绩
	display(ptPixelDatas,green_info,bound_x1,bound_x2,bound_y1,bound_y2);//lcd显示	
	return 0;
 ...
 * 函数名称： set_parity
 * 功能描述： 设置串口参数
 * 输入参数： fd 文件索引值 databits 数据位 stopbits停止位 parity检验位
 * 输出参数：  
 * 返 回 值： 0 - 成功, 负数 - 失败
 ***********************************************************************/
static int set_parity(int fd, int databits, int stopbits, int parity) 
...
/**********************************************************************
 * 函数名称： get_info
 * 功能描述： 将PT_PixelDatas的数据拷贝到PT_hand_pic中
 * 输入参数： PT_PixelDatas - 一个结构体,包涵读取摄像头的数据
			  PT_hand_pic - 一个结构体,包涵处理图片的主要参数
 * 输出参数：  
 * 返 回 值： 0 - 成功, 负数 - 失败
